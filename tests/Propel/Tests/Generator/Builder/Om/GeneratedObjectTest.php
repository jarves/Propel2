...
EOF;
    ...
    {
        //add countries
        $this->con->exec('SET FOREIGN_KEY_CHECKS=0; TRUNCATE country');
        $stmt = $this->con->prepare('INSERT INTO country VALUES (?, ?)');
        $stmt->execute(['fr', 'Paris']);
        $stmt->execute(['us', 'Washington']);
        $stmt->execute(['de', 'Berlin']);

        $this->con->exec('SET FOREIGN_KEY_CHECKS=0; TRUNCATE country_translation');
        $stmt = $this->con->prepare('INSERT INTO country_translation VALUES (null, ?, ?, ?)');
